...
<table width="100%"class="administrationTable">
 ...
 while($row = mysql_fetch_assoc($result)){
         ...
         <tr class="<?php echo (($row['enabled'] != '1') ? 'disabled' : (($i % 2 == 0) ? 'even' : 'odd'));?>">
          ...
          <td><a href="<?php echo $config['projectURL'];?>administration/editUser.html?id=<?php echo $row['id'];?>">edit user</a><?php if($row['enabled'] != '1'):?> | <a href="<?php echo $config['projectURL'];?>administration/users.html?action=enable&amp;id=<?php echo $row['id'];?>" onclick="return confirm('Do you want to enable this user?');">enable user</a><?php endif;?><?php if($row['isAdmin'] != '1'):?> | <a href="<?php echo $config['projectURL'];?>administration/users.html?action=delete&amp;id=<?php echo $row['id'];?>" onclick="return confirm('Do you really want to delete this user?');">delete user</a> | <a href="<?php echo $config['projectURL'];?>administration/users.html?action=login&amp;id=<?php echo $row['id'];?>">log in as user</a><?php endif;?></td>
         </tr>
         <?php
         $i++;
 }
