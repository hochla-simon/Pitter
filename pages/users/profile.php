...
if(isset($_POST['submit'])){
	...
	if(count($errors) == 0){
		$user = mysql_fetch_assoc($db->query("select * from users where email = '".mysql_real_escape_string($_POST['email'])."' and id != '".mysql_real_escape_string($currentUser['id'])."'"));
		if($user['id'] != ''){
			$errors[] = 'There is already another user having this email address.';
		}
	}
	if(count($errors) == 0){
		$db->query("update users set firstName = '".mysql_real_escape_string($_POST['firstName'])."', lastName = '".mysql_real_escape_string($_POST['lastName'])."', email = '".mysql_real_escape_string($_POST['email'])."', password = '".mysql_real_escape_string(crypt($_POST['password']))."' where id = '".mysql_real_escape_string($currentUser['id'])."'");
