 ...
 * Time: 10:46
 */
class TestMovePhoto extends PHPUnit_Framework_TestCase
{
    ...
    public function testMove(){
        ...
        $testInitialNumberOfImages = mysql_num_rows($result);
        ...
        //getting the number of images in the root album
        $result = $db->query('SELECT * FROM imagesToAlbums WHERE albumId=1');
        $rootInitialNumberOfImages = mysql_num_rows($result);
        ...
        $testAlbumImages = $db->query('SELECT * FROM imagesToAlbums WHERE albumId=' . self::$testAlbumId);

        $this->assertEquals($testInitialNumberOfImages+1,mysql_num_rows($testAlbumImages));

        //getting the new number of images in the root album
        $rootAlbumImages = $db->query('SELECT * FROM imagesToAlbums WHERE albumId=1');

        $this->assertEquals($rootInitialNumberOfImages-1,mysql_num_rows($rootAlbumImages));
    }
